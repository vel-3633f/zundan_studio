...
- 📚 **AI 台本生成**: お笑い漫談の台本を自動生成
  - 🎭 **長尺動画（5-10 分）**: 詳細なストーリー展開の漫談
  - ⚡ **ショート動画（60 秒）**: TikTok・YouTube Shorts 向けの高速生成
...
- 複数の API リクエストを同時に処理可能
- 1 つの重い処理（タイトル生成など）が実行中でも、他のリクエストがブロックされない
- 開発環境では`UVICORN_WORKERS`を設定しない（ホットリロード優先）
- 本番環境では`UVICORN_WORKERS=4`を推奨（CPU コア数に応じて調整）
