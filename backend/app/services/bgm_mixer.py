...
class BGMMixer:
    ...
    def _load_bgm_file(
        self, bgm_file_path: str, max_retries: int = 3
            ...
            for attempt in range(max_retries):
                try:
                    if attempt > 0:
                        ...
                        logger.warning(
                            f"BGMファイル読み込みリトライ ({attempt + 1}/{max_retries}): {bgm_file_path}"
                        )
                    ...
                    if attempt < max_retries - 1:
                        logger.warning(
                            f"BGMファイルの読み込み失敗 (試行 {attempt + 1}/{max_retries}): {bgm_file_path} - {str(e)[:100]}"
                        )
                        gc.collect()
                    else:
                        logger.error(
                            f"BGMファイルの読み込みエラー ({bgm_file_path}): {e}"
                        )
                        return None
    ...
    def create_section_bgm_track(
        self, section: VideoSection, start_time: float, duration: float
    ) -> Optional[AudioClip]:
        """セクション用のBGMトラックを作成
        ...
        try:
            ...
            if base_bgm_clip.duration < duration:
                ...
                for i in range(loop_count):
                    if i == 0:
                        loop_clips.append(
                            base_bgm_clip.subclipped(0, base_bgm_clip.duration)
                        )
                    else:
                        loop_clips.append(
                            base_bgm_clip.subclipped(0, base_bgm_clip.duration)
                ...
                bgm_clip = base_bgm_clip.subclipped(
                    0, min(duration, base_bgm_clip.duration)
                )
    ...
    def mix_bgm_with_voiceover(
        ...
        for i, (section, duration) in enumerate(zip(sections, section_durations)):
            ...
            bgm_clip = self.create_section_bgm_track(
                section, start_time=current_time, duration=duration
            )
