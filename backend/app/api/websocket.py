    ...
    try:
        while True:
            ...
            if task.state in ['SUCCESS', 'FAILURE', 'REVOKED']:
                logger.info(f"タスク完了、WebSocket接続を閉じます: task_id={task_id}, state={task.state}")
                break
        ...
        logger.error(f"WebSocketエラー: task_id={task_id}, error={str(e)}", exc_info=True)
        try:
