...
- **text_for_voicevox**: text を完全ひらがなに変換（**重要**: フィールド名は `text_for_voicevox` で、`voicevox` のスペルを正確に。`voivevox` などのタイポは絶対にしない）
...
- `visible_characters`: `["zundamon", "metan"]` のようにローマ字の配列
